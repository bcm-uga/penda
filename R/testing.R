...
#'simulation = simplified_simulation(simulation, fraction = 0.3, threshold = 60)
#'ctrl_data = ctrl_data[,-ncol(ctrl_data)-3:ncol(ctrl_data)]
#'step0(ctrl_data, simulation$simulated_data[,1], quant = 0.03)
#'
#'@export
...
#'ctrl_data = ctrl_data[,-ncol(ctrl_data)-3:ncol(ctrl_data)]
#'D_U_ctrl = find_D_U_ctrl(ctrl_data, quant = 0.001, factor = 4, threshold = 0.99)
#'test_patient1 = patient_test(ctrl_data, simulation$simulated_data[,1], quant_0 = 0.03, iterations = 10, D_U_ctrl, threshold = 0.2)
#'
#'@export

patient_test = function (ctrl_data, patient_genes, quant_0, iterations, D_U_ctrl, threshold, quant_test = 0, factor_test = 1){
...
#'patient_test_sansl0
#'
#' This function makes the test of the dysregulation of all genes for one patient, without the step0.
#'
#'@param ctrl_data A matrix with genes expressions in controls for all the patients.
#'@param patient_genes A vector with all the genes expressions for one patient.
#'@param iterations The maximal number of iterations for the test. If the dysregulation list
...
#'ctrl_data = ctrl_data[,-ncol(ctrl_data)-3:ncol(ctrl_data)]
#'D_U_ctrl = find_D_U_ctrl(ctrl_data, quant = 0.001, factor = 4, threshold = 0.99)
#'patient_test_sansl0(ctrl_data, simulation$simulated_data, iterations = 10, D_U_ctrl, threshold = 0.2)
#'
#'@export

patient_test_sansl0 = function (ctrl_data, patient_genes, iterations, D_U_ctrl, threshold, quant_test = 0, factor_test = 1){
