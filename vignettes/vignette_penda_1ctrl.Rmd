...
subtitle: "Special case - Performing personalized data analysis with `penda` and only one control sample"
...
The dataset used to illustrated the method corresponds to the transcriptomes of 3000 genes (RNAseq counts, normalized with DESeq2) for 1 normal, control samples and 40 tumorous samples taken from the TCGA study of lung adenocarcinoma [PMID:25079552].
...
```{r}
data_ctrl = penda::penda_data_ctrl[, 1]
head(data_ctrl)
```
...
data_case = data_case[names(data_ctrl),]
head(data_case[,1:3])
dim(data_case)
```
...
Penda_dataset = penda::make_dataset_1ctrl(data_ctrl, data_case)
...
The PenDA vignette of the specific version for one control of the `penda` package version 1.0 was executed on `r floor(Penda_dataset$info["init_nb_genes"])`  genes, using 1 control sample and `r floor(Penda_dataset$info["init_nb_cases"])` case samples.

1 control were used to generate L and H lists using the following parameters: s_max = `r s_max`.
