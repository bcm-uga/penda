...
The simulation vignette of the penda package version 1.0 was executed on XXX genes, using XXX control samples and XXX case samples.

The data set was pretreated as following: filtering for NA = XXX, filtering on low value = XXX based on XXX threshold, % of patient = XXX).

XXX cases samples were simulated using the complex simulation function with the following parameters: list size = XXX, grp size = XXX, threshold = XXX. Theses simulations identified XXX% of genes as typically deregulated in cases samples.

XXX controls were used to generate L and H lists using the following parameters: threshold = XXX and s_max = XXX

The quantile method was applied on these XXX cases. We retained a global FDR value of XXX, with the following set of parameters: quantile = XXX and factor = XXX.

The penda method was applied on these XXX cases. We retained a global FDR value of XXX, with the following set of parameters: quantile = XXX, factor = XXX and threshold = XXX.
