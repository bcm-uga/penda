...
The data set was pretreated as following: X
...
Genes and patients removed was under the conditions in `r Penda_dataset$info["threshold"]*100` % of cases.

`r ncol(data_simu)` cases samples were simulated using the complex simulation function with the following parameters: list size = XXX, grp size = XXX, threshold = XXX. Theses simulations identified XXX% of genes as typically deregulated in cases samples.
