...
The function `make_dataset` makes 3 steps to prepare the data at the analysis.  
  ...
  - `detect_na_value` function remove samples with more than `r "threshold_dataset"`% probes or patients with NA.  <br/>
...
For each, the list of **L** genes (resp. **H** genes) corresponds to the closest genes (up to `s_max` genes) that have a lower (resp. higher) expression in at least `r threshold_LH*100` % of the control samples. 
