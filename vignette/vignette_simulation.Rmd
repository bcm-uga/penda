...
<span style="color:red">Genes has to be ordered by their median in controls. </span>


```{r, results="verbatim"}
data_ctrl = penda::data_ctrl
median_gene = apply(data_ctrl, 1, median, na.rm = TRUE)
median_gene = sort(median_gene)
median_gene[10000:10050]

data_ctrl = data_ctrl[names(median_gene),]
...
The rownames of the matrix correspond the gene_symbol , the colnames indicate the sample ID.
Genes are ordered in the same order than in controls.
...
We filter the data with `detect_zero_value` to remove genes with very low expression. We used the function `normalmixEM` to define the threshold of low expression on all the datas normalized with a pseudo-log2.
You can also used the function `detect_na_value` to remove genes and patients with high rate of NA.

```{r}
all_data = c((log2(data_ctrl + 1)), (log2(data_case + 1)))
library(mixtools)
...
null_values = penda::detect_zero_value(data_ctrl, data_case, threshold = 0.99, min = val_min)
data_ctrl = data_ctrl[!null_values,] #filtered matrix
data_case = data_case[!null_values,] #filtered matrix
dim(data_ctrl)
dim(data_case)
```
...
#  which_threshold = penda::choose_threshold(data_ctrl, D_U_list, 30, simup1, threshold_values, quant_test = 0.01, factor_test = 1)
```
