...
df_quant = data.frame(quantile = factor(c(qf1[,1], qf15[,1], qf25[,1]))
                      , FDR = as.numeric(c(qf1[,2], qf15[,2], qf25[,2]))
                      , TPR = as.numeric(c(qf1[,3], qf15[,3], qf25[,3]))
                      , factor = factor(c(rep(c(1, 1.5, 2.5), each = 12)))
)
...
Here, we choose the factor 1 with the quantile 0.01. It allows to obtain a TPR > 0.8 with the lowest FDR.
