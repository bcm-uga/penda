...
The five last patients are used to makes "simu_data" and simulates the dysregulation. They are substrate of "ctrl_data" for not distort the next steps.
...
The function returns two matrix. For each gene in columns, the row gene is "TRUE" if is **D**own-expresed or **U**p-expressed.
...
#test_threshold = multiple_tests(ctrl_data, D_U_ctrl, 10, simulation, threshold_values, 0.03)
# saveRDS(test_threshold, "test_threshold.rds")
test_threshold = readRDS("test_threshold.rds")

#test_quantile = multiple_tests(ctrl_data, D_U_ctrl, 10, simulation, quantile_values, 0.2, threshold_change = FALSE)
