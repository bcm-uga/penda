...
data_simu = data_ctrl[, (ncol(data_ctrl)-1):ncol(data_ctrl)]
colnames(data_simu) = c("P1","P2","P3","P4","P5")

data_ctrl = data_ctrl[, -((ncol(data_ctrl)-1):ncol(data_ctrl))]
```
...
For example, an extract from `D_U_list` :
```{r, tidy=TRUE, tidy.opts=list(blank=FALSE, width.cutoff=55)}
#D_U_list = penda::compute_down_and_up_list(data_ctrl, threshold = 0.99, s_max = 50)

D_U_list$D[c(1,857, 940, 951, 802),120:130]
D_U_list$U[c(1,857, 940, 951, 802),120:130]
```
...
First parameters to test are those of the quantile method. This method is used in the Penda method when we don't have **D** or **U** list. There are two parameters, the `quantile` of the gene expression distribution in the control (dotted line), and the `factor` which modulate this quantile value (red line).
...
library(ggplot2)
ggplot(df_quant, aes(x = TPR, y = FDR, color = quantile, group = factor, shape = factor)) + geom_point() + geom_line()
...
if (!file.exists("tmp/which_threshold.rds")){
  which_threshold = penda::choose_threshold(data_ctrl, D_U_list, 30, simup1, threshold_values, quant_test = as.numeric(best_quantile$quantile), factor_test = as.numeric(best_quantile$factor))
